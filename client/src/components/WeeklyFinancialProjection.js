...
const WeeklyFinancialProjection = ({ userId, transactions }) => {
    const [advice, setAdvice] = useState([]);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);

    useEffect(() => {
        const cachedAdvice = localStorage.getItem('financialAdvice_weekly');
        if (cachedAdvice) {
    ...
    const handleGetAdvice = async () => {
        ...
        setError(null);
        try {
            const response = await axios.post('http://localhost:5000/api/financial/weekly-advice', { financialData: transactions });
            const { analysis, prediction, projection } = response.data.advice;
            const advice = [analysis, prediction, projection];
            setAdvice(advice);
            localStorage.setItem('financialAdvice_weekly', JSON.stringify({ advice }));
        } catch (error) {
    ...
    return (
        <Container>
            <h1>Weekly Financial Projection</h1>
            <Button onClick={handleGetAdvice} disabled={loading}>
                {loading ? `Fetching Advice...` : `Get Weekly Financial Advice`}
            </Button>
...
export default WeeklyFinancialProjection;
